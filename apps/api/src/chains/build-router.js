...
import { ChainRouter, EvmAdapter, EvmBalanceProvider, EvmPayoutBroadcaster, EvmRpc, FetchHttpClient, TronAdapter, TronBalanceProvider, TronPayoutBroadcaster, deriveEvmAddress, deriveTronAddress, makeTronSigner, } from "@cixtech/chains";
...
const EVM_CHAINS = ["POLYGON", "BSC", "ARBITRUM", "BASE"];
const NATIVE_SYMBOL = {
    POLYGON: "POL",
    BSC: "BNB",
    ARBITRUM: "ETH",
...
function evmTokenContracts(registry, env, chain) {
    const contracts = {};
 ...
 */
export function buildRouter(env, sql) {
    const accountXprv = env["CIXTECH_ENGINE_XPRV"];
    if (!accountXprv)
        throw new Error("CIXTECH_ENGINE_XPRV is required");
    ...
    const lookback = Number(env["CIXTECH_EVM_LOOKBACK_BLOCKS"] ?? String(DEFAULT_EVM_LOOKBACK_BLOCKS));
...
//# sourceMappingURL=build-router.js.map