...
export class DepositCursorStore {
    ...
    async get(chain, address) {
        const { rows } = await this.sql.query("SELECT next_block FROM deposit_cursor WHERE chain = $1 AND address = $2", [chain, address]);
        const raw = rows[0]?.next_block;
        return raw === undefined ? null : BigInt(raw);
    }
    async set(chain, address, nextBlock) {
        await this.sql.query(`INSERT INTO deposit_cursor (chain, address, next_block, updated_at)
       VALUES ($1, $2, $3, now())
       ON CONFLICT (chain, address)
       DO UPDATE SET next_block = EXCLUDED.next_block, updated_at = now()`, [chain, address, nextBlock.toString()]);
    }
}
//# sourceMappingURL=deposit-cursor.js.map