 ...
 *
 * Extracted from `AdminService` so both the admin console (on-demand) and the
 * background worker (scheduled) call the same logic. The sweep is a pure ledger
 * operation: it moves balances from `egofi_fee_revenue:*` / `pool_addr:*` to
 * `treasury:PLATFORM`, generating one journal entry per pool address swept.
 */
