...
| CX-13 | `@fastify/helmet` with a CSP including `frame-ancestors 'none'`, plus HSTS, `nosniff` and `no-referrer`. **Since the consoles were split into `apps/web` (see docs/DEPLOYMENT_TOPOLOGY.md), helmet no longer covers the console pages** — `apps/web/nginx.conf` sets their headers instead, with `connect-src` templated from `CIXTECH_API_BASE`. Both places must keep them. |
| CX-14 | `/metrics` requires the admin bearer token (constant-time compare) unless `CIXTECH_PUBLIC_METRICS=true`. |
| — | **New surface since the audit:** splitting the consoles out made every console call cross-origin, so the API now has a CORS allowlist (`CIXTECH_CORS_ORIGINS`). It is explicit with no wildcard and `credentials: false`, because `*` on an API that takes a bearer credential in a header would let any page use a stolen key from the victim's browser. Covered by tests in `apps/api/test/security-controls.test.ts`. |
