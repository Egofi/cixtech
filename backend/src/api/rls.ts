...
const TENANT_TABLES: readonly TenantTable[] = [
  ...
  // Human accounts. An operator row carries tenant_id NULL, so a tenant-scoped
  // read sees no operators at all — which is the intended answer: a tenant must
  // never enumerate our staff. Login runs outside a tenant scope (there is no
  // session yet), so the GUC is unset there and the lookup is unrestricted.
