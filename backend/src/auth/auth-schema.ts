/**
 * Human sessions, distinct from machine credentials.
 *
 * API keys (`api_key`) are for SERVERS: long-lived, scoped, no expiry, sent on
 * every request. That is correct for a tenant's backend calling the payout API,
 * and wrong for a person in a browser — a human pasting a long-lived bearer
 * credential into a page has no expiry, no idle timeout, no revocation short of
 * rotating the key for every integration that shares it, and no identity beyond
 * "whoever holds this string".
 *
 * These tables add the missing half. `api_key` is untouched and keeps doing its
 * job; everything here is about people.
 *
 * The two kinds of person share one `principal` table because they need
 * identical machinery — password, TOTP, sessions, lockout — and differ only in
 * which plane they reach and whether they belong to a tenant. Splitting them
 * would mean maintaining that machinery twice and having it drift.
 */
