import { type DepositSource, type WebhookPoster, verifyWebhook } from "@/chains";
import { Asset, type ChainDeposit, LedgerAccountKey } from "@/types";
...
describe("detection + webhook outbox", () => {
  ...
  it("retries a failing delivery with backoff, then dead-letters", async () => {
    const webhook = recordingPoster(false);
