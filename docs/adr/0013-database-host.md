...
- **Adopt Neon now for: dev, CI, and the egofi PSP app.** Its branching is
  ...
  testcontainers Postgres; Neon branching is an optional later convenience.)
...
- The transaction-mode pooling + RLS `SET LOCAL` interaction is a **[DECISION]**
  to verify before the ledger runs on any PgBouncer-fronted host, Neon included.
