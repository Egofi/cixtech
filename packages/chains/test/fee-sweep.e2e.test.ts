...
import { ExternalReconciler } from "../src/reconcile/external-reconciler.js";
import { FeeSweepPlanner } from "../src/treasury/fee-sweep-planner.js";
...
describe("a collected fee leaves the ledger and the chain in agreement", () => {
  ...
  it("reconciles clean after the fee actually moves", async () => {
    ...
    const { feeSwept } = await import("@cixtech/ledger");
    await ledger.post(
      feeSwept({
